...
export abstract class DataProviderFactory<V extends IDataprovider> {
  ...
  protected getCurrentDataProvider(): V {
    const currentDataprovider: V | undefined = this.dataProvidersSingleton.find(
      (dataProvider: V) =>
        this.getCurrentVersion() === dataProvider.getVersion(),
    );
    if (!currentDataprovider) {
      throw new Error(
        `No data provider found for version: ${this.currentVersion}`,
      );
    }

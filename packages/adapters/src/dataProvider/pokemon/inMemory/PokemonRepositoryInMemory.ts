...
export class PokemonRepositoryInMemory
  extends DataProvider
  implements IPokemonDataProviderBoundary
{
  ...
  getPokemon(id: string): Promise<Pokemon> {
    return new Promise((resolve, reject) => {
      const pokemonResolved: Pokemon | undefined = this.pokemons.find(
        (p: Pokemon) => p.id == id,
      );
      if (pokemonResolved) {
        ...
        reject(
          new Error(`pokemon by id ${id} doesn't exist in this repository`),
        );
      }
    });
  }

...
describe("PokemonRepositoryHttp", () => {
  ...
  it("should get pokemons", async () => {
    ...
    jest
      .spyOn(PokemonRepositoryHttp.prototype, "getPokemons")
      .mockImplementation(() => {
  ...
  it("should get pokemon by id", async () => {
    ...
    jest
      .spyOn(PokemonRepositoryHttp.prototype, "getPokemon")
      .mockImplementation(() => {
        return httpClient.get(
          `http://localhost:8000/pokemons/${pokemonFound.id}`,
        );
      });
    ...
    // When
    const pokemon: Pokemon = await pokemonRepository.getPokemon(
      pokemonFound.id,
    );
  ...
  it("should throw error when pokemon doesn't exist", () => {
    ...
    const httpClient: HttpClient = {
      get(url: string): Promise<any> {
        ...
        return Promise.reject(
          new Error("999 doesn't exist in this repository"),
        );
      },
    } as HttpClient;
    ...
    expect(request).rejects.toThrowError(
      "999 doesn't exist in this repository",
    );
  });

  it("should add new pokemon", async () => {
    // Given
    const newPokemon: Pokemon = Builder<Pokemon>()
      .name("Bulbizarre")
      .id("3")
      .build();
    ...
    jest
      .spyOn(PokemonRepositoryHttp.prototype, "addPokemon")
      .mockImplementation(() => {
    ...
    const addedPokemon: Pokemon =
      await pokemonRepository.addPokemon(newPokemon);
  ...
  it("should delete pokemon", async () => {
    ...
    const httpClient: HttpClient = {
      delete(url: string): Promise<any> {
        expect(url).toBe(
          `http://localhost:8000/pokemons/${pokemonToDelete.id}`,
        );
      ...
      get(url: string): Promise<any> {
        expect(url).toBe(`http://localhost:8000/pokemons`);
        return Promise.resolve(
          stubPokemons.filter((p) => p.id !== pokemonToDelete.id),
        );
      },
    } as HttpClient;

    jest
      .spyOn(PokemonRepositoryHttp.prototype, "deletePokemon")
      .mockReturnValue(
        httpClient.delete(
          `http://localhost:8000/pokemons` + `/${pokemonToDelete.id}`,
        ),

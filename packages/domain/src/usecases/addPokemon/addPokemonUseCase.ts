...
export class AddPokemonUseCase
  extends AbstractAddEditUseCase<InputAddPokemonValues, OutputAddPokemonValues>
  implements IAddPokemonEntryPointBoundary
{
  constructor(
  ...
  override async execute(
    inputAddPokemonValues: InputAddPokemonValues,
  ): Promise<OutputAddPokemonValues> {
    const errors: Map<PokemonFields, string> = await this.validate(
      inputAddPokemonValues.pokemonRequest,
    );
    ...
    if (!errors.size) {
      ...
      const addedPokemon: Pokemon =
        await this.pokemonDataProvider.addPokemon(pokemon);
      return Promise.resolve(
        Builder<OutputAddPokemonValues>().pokemon(addedPokemon).build(),
      );
    }

...
describe("Get Pokemons Use Case", () => {
  it("returns an empty list when no pokemons are available", async () => {
    const pokemonRepository: IPokemonDataProviderBoundary =
      Builder<IPokemonDataProviderBoundary>()
        ...
        .build();
    const useCase: GetPokemonsUseCase = new GetPokemonsUseCase(
      pokemonRepository,
    );

    const outputGetPokemonsValues: OutputGetPokemonsValues =
  ...
  it("display list of pokemons", async () => {
    ...
    useCase
      .execute()
      .then((outputGetPokemonsValues: OutputGetPokemonsValues) => {
        // Then
        expect(outputGetPokemonsValues.pokemons).toHaveLength(2);
        expect(outputGetPokemonsValues.pokemons).toContainEqual(
          expect.objectContaining({ name: "Pikachu" }),
        );
        expect(outputGetPokemonsValues.pokemons).toContainEqual(
          expect.objectContaining({ name: "Carapuce" }),
        );
      });
  });
  ...
  it("throws an error when repository fails to fetch pokemons", async () => {
    ...
    const pokemonRepository: IPokemonDataProviderBoundary =
      Builder<IPokemonDataProviderBoundary>()
        .getPokemons(() => Promise.reject(error))
        .build();
    const useCase: GetPokemonsUseCase = new GetPokemonsUseCase(
      pokemonRepository,
    );

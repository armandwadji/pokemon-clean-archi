...
export abstract class AbstractAddEditUseCase<I extends InputValues, O extends OutputValues> extends UseCase<I, O>{
    ...
    validate(pokemonRequest: PokemonRequest) : Promise<Map<PokemonFields, string>>  {
        let errorsMessage = '';
        const errors = new Map<PokemonFields, string>();
        const nameRegex: boolean = new RegExp('^[\\p{L}0-9_.\\s]{1,10}$', 'u').test(String(pokemonRequest.name));
        ...
        const pictureRegex: boolean = new RegExp('^https://assets.pokemon.com/assets/cms2/img/pokedex/detail/[0-9]{3}.png$').test(String(pokemonRequest.picture));

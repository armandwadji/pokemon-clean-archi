...
describe('pokemonResolver', () => {
  ...
  const executeResolver: ResolveFn<MaybeAsync<Pokemon>> = (
    ...resolverParameters
  ) =>
  ...
  it('should get pokemon by id', () => {
    ...
    of(executeResolver(activatedRouteSnapshot, stateSnapshotMock)).subscribe(
      (pokemon) => {
  ...
  it('should throw error if id is empty', async () => {
    ...
    expect(mockLogger.error).toHaveBeenCalledWith(
      'Pokemon ID is required for the resolver',
    );
  });
});

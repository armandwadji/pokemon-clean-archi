...
describe('LoggerInterceptor', () => {
  ...
  describe('customiseRequestBody', () => {
    ...
    ])('should customize request body with log $level to $expected', ({level, expected}) => {
      // GIVEN
  ...
  describe('alterHttpRequest', () => {
    ...
    beforeEach(()=> {
      mockRequest = new HttpRequest('POST', 'https://example.com/api', {body: { key: 'value' }});
    })
    ...
    it('should add Authorization headers when both values are available', () => {
      ...
      // THEN
      expect(result.headers.get('Authorization')).toBe('Bearer XXXX-XXXX-XXXX-XXXX');

...
const PokemonForm: FunctionComponent<Props> = ({ id, pokemon }) => {
  ...
  useEffect(() => {
    getPokemonsController
      .getPokemons()
      .then((pokemonPresenter: PokemonsPresenterVM) =>
        setTypes(pokemonPresenter.pokemonTypes ?? []),
      );
  }, [getPokemonsController]);
  ...
  const addPokemon = (pokemon: PokemonRequest): void => {
    addPokemonController
      .create(pokemon)
      .then(() => navigate(`/${routesName.pokemon.path}`));
  };
  ...
  const validateForm = async (): Promise<boolean> => {
    ...
    const [pictureError, nameError, hpError, cpError]: (string | undefined)[] =
      await Promise.all([
                ...
                {isAddForm && (
                  <div className="form-group">
                    ...
                    <input
                      ...
                      defaultValue={form.picture.value}
                      name="picture"
                    ...
                    {form.picture.error && (
                      <div
                        ...
                        dangerouslySetInnerHTML={{ __html: form.picture.error }}
                      ></div>
                    )}
                  </div>
                )}

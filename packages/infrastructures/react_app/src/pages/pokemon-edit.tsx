...
const PokemonEdit: FunctionComponent = () => {
    ...
    const [pokemonRequest, setPokemonRequest] = useState<PokemonRequest | undefined>(undefined);

    useEffect(() => {
        if (pokemon) {
            const {name, hp, cp, picture, types, created} = pokemon;
            setPokemonRequest(new PokemonRequest(hp, cp, name, picture, types, created));
        }
    }, [pokemon]);
